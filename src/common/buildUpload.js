  ...
  fs.readdir(filePath, function(err, files) {
    files.forEach(function(item, index) {
      ...
      if (fs.statSync(currentPath).isFile()) {
        ...
        fileUpload(receiver, {}, { to: fullToPath }, fileCon, item, function(res, body) {
          console.log('【' + padStart(date.getHours(), 2, '0') + ':' + padStart(date.getMinutes(), 2, '0') + ':' + padStart(date.getSeconds(), 2, '0') + '】  ' + resolvePath + '>>>>' + fullToPath)
          // console.log(res)
          // console.log(body)
        })
